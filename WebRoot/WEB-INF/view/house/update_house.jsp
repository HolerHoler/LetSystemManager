...
<body>
	<article class="page-container"> <s:form namespace="/action"
		action="updatedHouse" class="form form-horizontal" method="post">
		<s:hidden name="house.id" value="%{house.id}" />
		<div class="row cl">
		...
		<div class="row cl">
			<div class="col-xs-8 col-sm-9 col-xs-offset-4 col-sm-offset-3">
				<!-- 	<input type="button" id="updatedHouse_0" value="更新"
					class="btn btn-primary radius" /> -->
				<input class="btn btn-primary radius" type="submit" value="提交">
			</div>
		</div>
	</s:form> </article>
	<%-- 	<s:form namespace="/action" action="updatedHouse">
		<s:hidden name="house.id" value="%{house.id}" />
		<s:textfield name="house.houseName" label="房子名字" />
		<s:select label="房子类型" headerKey="0" headerValue="请选择房子类型"
			list="#{'普通':'1-普通', '别墅':'2-别墅'}" name="house.type" listKey="key"
			listValue="value" />
...
<%-- <script type="text/javascript">
	$(document).ready(function() {
		$("#updatedHouse_0").click(function() {
			//	$("#updatedLet").ajaxSubmit();

			$("#updatedHouse").ajaxSubmit(function(result) {
